...
type Props = {
  ...
  onClose: () => void
  /**
   * 見出しと閉じるボタンを出すか。既定は出す（モーダルでの利用が主）。
   * `ScreenFrame` が既にタイトルを出すフルページ版（`AddLogView`）では
   * false にして二重にならないようにする。
   */
  showHeader?: boolean
}

/**
 * 発言の投稿と編集。フォーム本体だけを持つ（`ScreenFrame` は含まない）。
 *
 * `AddLogView`（URLに直接来たときのフルページ版）と `UnitView` の
 * モーダルの両方から使う。`logId` があれば編集、無ければ新規。
 * フォームの中身が同じなので、画面を2つに分けず1つで両方を受け持っている。
 */
      ...
      {showHeader && (
        <div className="modal-head">
          <h2 className="modal-title">
            {isEditing ? '発言を編集' : '発言を追加'}
          </h2>
          <button
            type="button"
            className="quiet-button subtle modal-close"
            onClick={onClose}
            aria-label="閉じる"
          >
            ×
