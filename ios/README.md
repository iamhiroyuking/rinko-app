...
- **招待リンクの参加**: カスタムURLスキーム `rinko://join/<token>` を受ける。
  ユニバーサルリンク（`https://.../join/<token>`）はまだ未対応——
  Associated Domains・Apple DeveloperのTeam ID・サーバー側の
  `apple-app-site-association` 配置が要るため。手動貼り付け
  （`AddBookScreen` の「招待リンクで参加」タブ）が今のいちばん確実な入口
- **表紙画像**: 選ぶ→長辺800pxに縮小→JPEG化までは `ImagePicking.swift`。
  記録の添付（長辺1600px）と上限だけ変えて同じ考え方
