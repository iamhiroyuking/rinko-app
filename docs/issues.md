# Issue一覧（v1着手時点の実装計画・過去の記録）

> **これは現在の計画ではない。** v1に着手する前（2026-08時点）に立てた最初の29件の計画で、
> 実際に作業した GitHub Issue（#55〜、現在は#100番台）とは対応しない。
> 実際の状況は [CLAUDE.md](../CLAUDE.md) の「進行状況・次のステップ」と、
> リポジトリの [Issue一覧](https://github.com/iamhiroyuking/rinko-app/issues) を見ること。
>
> ここに残しているのは「どう割ったか」という考え方の記録。特に下の
> 「割り方の方針」（縦切り／横切り）は今も通用する。M0・M1のチェックボックスも
> 実績として残しておき、それ以降（M2〜M4）は当時の見込みに過ぎない。
...
> **進捗（2026-08-11）**: M0（#1〜#4）は完了。GitHubのIssue番号は
> #1〜#4 がM0、#10〜#15 がM1に対応する（この文書の #5〜#10 がGitHubの #10〜#15）。
...
- [ ] 削除した人が残したログは残る
- [ ] **回の削除は作成者のみ。** 削除すると全員の画面から消えるが、ゴミ箱に出て復元できるのは作成者だけ
- [ ] 作成者以外には回の削除ボタンを出さない（データベース側はトリガーで守られている）

依存: #17

### #19 ログにしおりを付けて見返せる
- [ ] ログのメニューからしおりを付け外しできる
- [ ] しおりを付けたログが見分けられる
- [ ] **他人のしおりは見えない**（`log_marks` テーブル）
