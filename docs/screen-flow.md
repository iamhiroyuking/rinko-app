...
| JoinBookView | `/join/<token>` を開いたときの参加処理。設計段階には無かったが、リンクを押して参加できる形にするため追加した |
...
- 各枠の右上の編集ボタンから、その回の編集・削除を行う。**編集は参加者全員に同期し、削除は作成者のみ**
- 「次回はここ」のハイライトは行わない
...
- ログの右上のメニューからしおりを付ける。後から見返しやすくするための**個人の目印で、他人には見えない**
- ステータスは参加者なら誰でも変更できる
...
### TrashView
自分がゴミ箱に入れた教材と、自分が作って削除した回が入る。復元すると元に戻る。
他人が作った回は、自分の画面から消えていてもここには出ない（復元できるのは作成者だけ）。
