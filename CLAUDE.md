...
**Markdown対応は入れていない。** 改行は `white-space: pre-wrap` で既に効いて
おり、新しく得られるのは箇条書き・強調・見出し程度。一方でバンドルが
圧縮後143KBから3割ほど増える。**動機だった数式はMarkdownでは出ない**
（KaTeXが別に要る）し、そこは画像添付で既に解けている。
実際に「箇条書きが書けなくて困った」と感じてから入れる。
