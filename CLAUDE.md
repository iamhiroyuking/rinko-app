...
- `join_book_with_token()` — 招待リンクでの参加
- `delete_orphan_book()` — 参加者ゼロの教材が消える
- 参加していない人には教材・回・ログ・タグ・招待リンクのすべてが見えない（3アカウントで確認）
...
### 開発用アカウント
`devtest@example.com` / `devtest2@example.com` / `devtest3@example.com` を動作確認用に作成してある。
（順に 作成者 / 招待される人 / 招待されていない人 として権限の検証に使った）**公開前に削除する。**
本人のアカウント `ひろゆき` は残す。
