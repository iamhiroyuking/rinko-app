...
### 開発用アカウント
`devtest@example.com` / `devtest2@example.com` / `devtest3@example.com`
（順に 作成者 / 招待される人 / 招待されていない人 として権限の検証に使った。パスワードは共通）

**検証用データは削除済み**（教材1件と配下のログ・タグ・招待リンク）。
アカウント自体は残っている。**削除には管理者権限が必要なので本人がSupabaseの管理画面から行う。**
本人のアカウント `ひろゆき` は残す。
